                    ...
                    <div className="panel panel-default" key={'p' + variable}>
                        ...
                        <div id={variable + variable} className="panel-collapse collapse" role="tabpanel" aria-labelledby={variable}>
                            <div className="panel-body">
                                {repo[variable].map(function(body) {
                                    return (
                                        <div key={body}>
...
function out(f){
  for (var i = 0; i < f.length; i++) {
    for (var u = 0; u < lan.length; u++) {
       if(f[i].id == lan[u].id){
          f[i]['title'] = lan[u].title;
          f[i].body.map(function(x){
             return x.push((lan[u].menu[x%100-1]));
          })
       }
    }
  }
